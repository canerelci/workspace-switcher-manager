...
- **Popup sizing:** **`popup-height-scale`** divides the aspect-derived tile dimension (horizontal strip → shorter tiles when &gt;100). **`popup-label-padding-scale`** scales horizontal **`em`** padding on labels (default **55** ≈ tighter than legacy **100**). Label **`font-size`** uses **`_wsmEffectiveFontFit()`**: **`_fitToScreenScale`** × (**allocated tile width** / **`WSM_FONT_CARD_WIDTH_REF_PX`**, clamped), with a coarse guess until **`_childWidth`** exists.
