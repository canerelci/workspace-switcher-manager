...
- **Popup sizing:** **`popup-height-scale`** divides the aspect-derived tile dimension (horizontal strip → shorter tiles when &gt;100). **`popup-label-padding-scale`** scales horizontal **`em`** padding on labels (default **55** ≈ tighter than legacy **100**). Label **`font-size`** uses **`_wsmEffectiveFontFit()`** (shrink-to-fit × short-side boost), not raw **`_fitToScreenScale`** alone.
